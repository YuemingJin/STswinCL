...
def parse_option(stage='pre-train'):
    ...
    parser.add_argument('--crop', type=float, default=0.08 if stage == 'pre-train' else 0.08, help='minimum crop')
    parser.add_argument('--aug', type=str, default='BYOL',
                        choices=['NULL', 'InstDisc', 'MoCov2', 'SimCLR', 'RandAug', 'BYOL', 'val'],
                        help='which augmentation to use.')
    parser.add_argument('--zip', action='store_true', default='no', help='use zipped dataset')
    ...
    parser.add_argument('--data', type=str, default='endo18')
    parser.add_argument('--tag', type=str, default='1')
    parser.add_argument('--pretrainpth', type=str, default='swinPlusv2_ver_9/epoch_9_checkpoint.t7')
    parser.add_argument('--ann-file', type=str, default='', help='annotation file')
    parser.add_argument('--image-size_h', type=int, default=256, help='image crop size')
    parser.add_argument('--image-size_w', type=int, default=448, help='image crop size')
    ...
    if stage == 'linear':
        ...
        parser.add_argument('--batch-size', type=int, default=2, help='batch_size for single gpu')

    # model
    parser.add_argument('--arch', type=str, default='resnet50', choices=model_names, help="backbone architecture")
    if stage == 'pre-train':
        parser.add_argument('--model', type=str, default='PixPro',help='which model to use')
    ...
    # optimization
    if stage == 'pre-train':
        parser.add_argument('--base-learning-rate', '--base-lr', type=float, default=1.0,
    ...
    parser.add_argument('--weight-decay', type=float, default=1e-5 if stage == 'pre-train' else 0, help='weight decay')
    ...
    # misc
    parser.add_argument('--output-dir', type=str, default='./CL_output', help='output director')
    ...
    # PixPro arguments
    if stage == 'pre-train':
        parser.add_argument('--pixpro-p', type=float, default=2.)
        parser.add_argument('--pixpro-momentum', type=float, default=0.99, help='momentume parameter used in MoCo and InstDisc')
        parser.add_argument('--pixpro-pos-ratio', type=float, default=5.0, help='maximum distance ratio of positive patches')
