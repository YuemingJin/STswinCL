...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(torch.load("/research/pheng3/ymjin/F1_scene/code/net_weights/resnet50-19c8e357.pth"))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/wjc/.cache/torch/checkpoints/resnet101-5d3b4d8f.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/wjc/.cache/torch/checkpoints/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/wjc/.cache/torch/checkpoints/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/wjc/.cache/torch/checkpoints/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            resnet = models.resnet18()
            # load pretrained model:
            # resnet.load_state_dict(torch.load("/data/ymjin/Server_data1/F1_scene/code/net_weights/resnet18-5c106cde.pth"))
            # resnet.load_state_dict(torch.load("/home/ymjin/Server_data1/F1_scene/code/net_weights/resnet18-5c106cde.pth"))
            #resnet.load_state_dict(torch.load("/research/pheng3/ymjin/F1_scene/code/net_weights/resnet18-5c106cde.pth"))
            ...
            print ("pretrained resnet, 18")
            #print('/research/pheng3/ymjin/F1_scene/code/net_weights/resnet18-5c106cde.pth')
        elif num_layers == 34:
            resnet = models.resnet34()
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/wjc/.cache/torch/checkpoints/resnet34-333f7ec4.pth"))
