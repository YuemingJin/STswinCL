...
def regression_loss(q, k, adj1, adj2, adj3,neg3, label_patch1, label_patch2, label_adj1, label_adj2, label_adj3,label_neg3,class_num):
    N, C, H, W = q.shape
    q = q.view(N, C, -1) #N,C,HW: 2,256,784
    ...
    logit1 = torch.bmm(q.transpose(1, 2), k) #N,HW,C * N,C,HW = N,HW,HW: 2,784,784
    ...
    mask_p = posMask(label_patch1, label_patch2,class_num).cuda().detach() #2,784,784
    ...
    P_exp = torch.exp(P) #2,784
    #print('P_exp shape:', P_exp.shape)
    N_exp = torch.exp(N)
...
class ConsistencyLoss(nn.Module):
    def __init__(self, args):
        ...
        # self.segmentor = DeepLabV3(12, backbone='resnest50', aux=False).cuda()
        # self.segmentor.load_state_dict(torch.load(os.path.join(dirname, 'checkpoint.pth.tar'))['state_dict'])
    ...
    def forward(self, im_1, im_2, im_3, im_4, im_5, im_6,mask_1, mask_2, mask_3, mask_4, mask_5,mask_6):
        ...
        #print('mask shape:', mask_1.shape) #2,1,28,28
        #print(1 + '1')
