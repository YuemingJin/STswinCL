...
def patch_grid(transform, shape=(64, 64, 3), stride=[1, 1]):
...
def get_frame_transform(frame_transform_str, img_size):
    ...
    if 'cj' in fts:
        _cj = 0.1
        color_jitter = transforms.ColorJitter(brightness=0.8, contrast=0.8, saturation=0.8, hue=0.05)
        tt += [transforms.RandomApply([color_jitter], p=0.8),]
